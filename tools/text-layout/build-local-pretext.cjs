...
const pretextRoot = path.join(projectRoot, 'site', 'pretext-main');
...
const tsRuntimeCandidates = [
  ...
  path.join(projectRoot, 'tools', 'qa', 'node_modules', 'typescript', 'lib', 'tsserverlibrary.js'),
  path.join(
...
function main() {
  ...
  if (!files.length) {
    const fallbackEntry = path.join(distDir, 'layout.js');
    if (!fs.existsSync(fallbackEntry)) {
      throw new Error('pretext 既没有可重建的 src 目录，也没有可直接使用的 dist/layout.js。');
    }
