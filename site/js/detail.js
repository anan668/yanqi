...
function injectSpotMapDataForDetailItem(record, fallbackId) {
    const normalizedFallbackId = Number(fallbackId);
    const candidateId = Number(record?.id);
    const mapRecord = getMapCatalogSpotById(Number.isFinite(candidateId) ? candidateId : normalizedFallbackId);
...
function normalizeDetailRelatedSpotData(record) {
    const mergedRecord = injectCatalogIdentityForDetailItem(record, record?.id);

    return {
        ...mergedRecord,
        price: getSpotBasePriceText(mergedRecord?.id, mergedRecord?.price)
    };
}

function normalizeDetailSpotDataRecord(record, fallbackId) {
    const mergedRecord = injectSpotMapDataForDetailItem(
        injectCatalogIdentityForDetailItem(record, fallbackId),
        fallbackId
    );

    return {
        ...mergedRecord,
        priceFrom: getSpotBasePriceText(fallbackId, mergedRecord?.priceFrom),
        itineraries: Array.isArray(mergedRecord?.itineraries)
            ? mergedRecord.itineraries.map((item) => ({
                ...item,
                price: normalizeDisplayPriceText(item.price)
            }))
            : [],
        related: Array.isArray(mergedRecord?.related)
            ? mergedRecord.related.map((item) => normalizeDetailRelatedSpotData(item))
            : []
    };
}
...
const detailSpotDataLoadPromiseCache = new Map();
const detailSpotDataCache = new Map();
let leafletLibraryPromise = null;

function getDetailSpotDataManifestEntry(spotId) {
    return sharedDetailSpotDataManifest && typeof sharedDetailSpotDataManifest.getById === 'function'
        ? sharedDetailSpotDataManifest.getById(spotId)
        : null;
}

function getRawDetailSpotDataById(spotId) {
    return sharedDetailSpotDataRegistry && typeof sharedDetailSpotDataRegistry.getById === 'function'
        ? sharedDetailSpotDataRegistry.getById(spotId)
        : null;
}

function buildBootstrapDetailSpotData(spotId) {
    const fallbackSpot = getCatalogSpotById(spotId) || getCatalogSpotById(1);
    const fallbackSpotId = fallbackSpot?.id || 1;
    const bootstrapPayload = window.__YANQI_DETAIL_BOOTSTRAP__ || null;
    const activeBootstrap = Number(bootstrapPayload?.id) === fallbackSpotId ? bootstrapPayload : null;
    ...
    return normalizeDetailSpotDataRecord({
        ...
        season: activeBootstrap?.season || fallbackSpot?.season || '',
        priceFrom: activeBootstrap?.priceFrom || '',
        mapLocation: '',
        coordinates: '',
        features: {
            location: '',
...
function getLoadedDetailSpotDataById(spotId) {
    const normalizedSpotId = Number(spotId);
    if (!Number.isFinite(normalizedSpotId)) {
        return null;
    }

    const cachedData = detailSpotDataCache.get(normalizedSpotId);
    if (cachedData) {
        return cachedData;
    }

    const rawSpotData = getRawDetailSpotDataById(normalizedSpotId);
    if (!rawSpotData) {
        return null;
    }

    const normalizedSpotData = normalizeDetailSpotDataRecord(rawSpotData, normalizedSpotId);
    detailSpotDataCache.set(normalizedSpotId, normalizedSpotData);
    return normalizedSpotData;
}

function loadDetailSpotData(spotId) {
    const normalizedSpotId = Number(spotId);
    if (!Number.isFinite(normalizedSpotId)) {
        return Promise.reject(new Error(`Invalid detail spot id: ${spotId}`));
    }

    const loadedSpotData = getLoadedDetailSpotDataById(normalizedSpotId);
    if (loadedSpotData) {
        return Promise.resolve(loadedSpotData);
    }

    if (detailSpotDataLoadPromiseCache.has(normalizedSpotId)) {
        return detailSpotDataLoadPromiseCache.get(normalizedSpotId);
    }

    const manifestEntry = getDetailSpotDataManifestEntry(normalizedSpotId);
    if (!manifestEntry?.src) {
        return Promise.reject(new Error(`Missing detail spot data manifest entry for spot ${normalizedSpotId}.`));
    }

    const loadPromise = new Promise((resolve, reject) => {
        const resolveLoadedSpotData = () => {
            const nextSpotData = getLoadedDetailSpotDataById(normalizedSpotId);
            if (nextSpotData) {
                resolve(nextSpotData);
                return;
            }

            reject(new Error(`Detail spot data loaded without registry payload for spot ${normalizedSpotId}.`));
        };

        let script = document.querySelector(`script[${DETAIL_SPOT_DATA_SCRIPT_ID_ATTRIBUTE}="${normalizedSpotId}"]`);
        const handleLoad = () => {
            script?.setAttribute('data-detail-spot-ready', 'true');
            resolveLoadedSpotData();
        };
        const handleError = () => {
            script?.remove();
            reject(new Error(`Failed to load detail spot data script: ${manifestEntry.src}`));
        };
        ...
        if (!script) {
            ...
            script.async = true;
            script.setAttribute(DETAIL_SPOT_DATA_SCRIPT_ID_ATTRIBUTE, String(normalizedSpotId));
            script.setAttribute(DETAIL_SPOT_DATA_SCRIPT_SRC_ATTRIBUTE, manifestEntry.src);
        ...
        if (script.getAttribute('data-detail-spot-ready') === 'true') {
            resolveLoadedSpotData();
            return;
        }
        ...
        detailSpotDataLoadPromiseCache.delete(normalizedSpotId);
    });

    detailSpotDataLoadPromiseCache.set(normalizedSpotId, loadPromise);
    return loadPromise;
}

function warmDetailSpotData(spotId) {
    return loadDetailSpotData(spotId).catch(() => null);
}
...
function loadLeafletLibrary() {
    ...
    if (leafletLibraryPromise) {
        return leafletLibraryPromise;
    }

    leafletLibraryPromise = new Promise((resolve, reject) => {
        let script = document.head?.querySelector('script[data-yanqi-leaflet-script="true"]') || null;
        const handleLoad = () => {
            if (window.L) {
                resolve(window.L);
                return;
        ...
        const handleError = () => {
            script?.remove();
            reject(new Error(`Failed to load ${DETAIL_LEAFLET_SCRIPT_SRC}.`));
        };

        if (!script) {
            script = document.createElement('script');
            script.src = DETAIL_LEAFLET_SCRIPT_SRC;
            script.async = true;
            script.setAttribute('data-yanqi-leaflet-script', 'true');
            script.addEventListener('load', handleLoad, { once: true });
            script.addEventListener('error', handleError, { once: true });
            document.head?.appendChild(script);
            return;
...
/**
 * navigateWithDepth(url, options) - 统一通过深度管理器执行跨页导航
...
class DetailPage {
    ...
    constructor() {
        ...
        this.seaAtlasInlineLoadRequestKey = '';
        ...
        this.seaAtlasFullscreenLoadRequestKey = '';
    ...
    async init() {
        try {
            await this.ensurePrimarySpotDataReady();
        } catch (error) {
     ...
     * @returns {void}
     */
    async swapSpotContentInDocument(targetId, options = {}) {
        ...
        let nextSpotData = null;
        try {
            nextSpotData = await loadDetailSpotData(targetId);
        } catch (error) {
            console.error('[Yanqi detail] Failed to load in-document target spot data:', error);
        }
    ...
    startInDocumentDetailSwap(targetId, options = {}) {
        ...
        const commitSwap = async () => {
            this.inDocumentDetailSwapTimer = 0;
            await this.swapSpotContentInDocument(targetId, {
                direction,
                updateHistory,
                entryClass
            });
        ...
        Promise.all([
            warmDetailSpotData(targetId),
            new Promise((resolve) => {
                this.inDocumentDetailSwapTimer = window.setTimeout(resolve, DETAIL_SWAP_NAVIGATE_DELAY_MS);
            })
        ]).then(() => {
            commitSwap().catch(() => {
                this.isInDocumentDetailSwapping = false;
    ...
    clearBookingFocusPanelReturnMotion() {
        ...
        this.bookingFocusPanel?.classList.remove('is-returning', 'is-pulsing', 'is-receiving-modal-return');
    }
    ...
    requestSeaAtlasMapScene(options = {}) {
        const {
            animate = false,
            target = 'inline',
            viewState = null
        ...
        const requestKey = `${this.spotId}:${target}:${Date.now()}`;
        if (target === 'fullscreen') {
            this.seaAtlasFullscreenLoadRequestKey = requestKey;
        } else {
            this.seaAtlasInlineLoadRequestKey = requestKey;
        }

        return loadLeafletLibrary()
            .then(() => {
                const activeRequestKey = target === 'fullscreen'
                    ? this.seaAtlasFullscreenLoadRequestKey
                    : this.seaAtlasInlineLoadRequestKey;
                const shouldSkipClosedFullscreen = target === 'fullscreen' && !this.seaAtlasFullscreenOpen;

                if (activeRequestKey !== requestKey || shouldSkipClosedFullscreen || !this.seaAtlasCurrentMapData) {
                    return false;
                }
            ...
            .catch((error) => {
                console.error('[Yanqi detail] Failed to load Leaflet for sea atlas:', error);
                this.showSeaAtlasFallback('init', target);
                return false;
            });
    }
    ...
    awakenSeaAtlasShell(shell = null, options = {}) {
        ...
        if (targetShell.classList.contains('is-map-awake')) {
            if (this.activeSeaView === 'location') {
                this.requestSeaAtlasMapScene({
                    target: 'inline',
    ...
    renderMapInfo(options = {}) {
        ...
        const shouldWarmInlineMapNow = this.activeSeaView === 'location' && Boolean(this.seaAtlasMap);
        if (shouldWarmInlineMapNow) {
            this.requestSeaAtlasMapScene({
    ...
    closeBookingModal() {
        ...
        if (canReverseMorph) {
            ...
            if (
                ...
                const isFocusPanelTarget = Boolean(
                    sourceElement &&
                    this.bookingFocusPanel &&
                    sourceElement === this.bookingFocusPanel
                );
                ...
                this.bookingModalCloseTimer = window.setTimeout(() => {
                    ...
                    this.clearBookingModalMorph({
                        preserveFocusReturnMotion: isFocusPanelTarget
                    });
                    this.bookingModalCloseTimer = 0;
                    this.syncOverlayLock();
                }, 760);
    ...
    buildRelatedStageMarkup(direction = '') {
        ...
        const activeImage = activeSpot.image;
    ...
    preloadRelatedAssets(relatedSpots) {
        ...
        const warmSpot = (spot) => {
            ...
            preloadSpotVisual(spot);
            warmDetailSpotData(spot.id);
        };

        const [primarySpot, ...secondarySpots] = relatedSpots;
        if (primarySpot) {
            const queuePrimaryWarm = typeof window.requestIdleCallback === 'function'
                ? () => window.requestIdleCallback(() => warmSpot(primarySpot), { timeout: 900 })
                : () => window.setTimeout(() => warmSpot(primarySpot), 180);
            queuePrimaryWarm();
        }

        secondarySpots.forEach((spot) => {
            const registerPointerWarmup = (selector) => {
                const node = this.relatedGrid?.querySelector(selector);
                if (!node) {
                    return;
                }
                ...
                const handleWarmup = () => {
                    warmSpot(spot);
                    node.removeEventListener('pointerenter', handleWarmup);
                    node.removeEventListener('focusin', handleWarmup);
                };

                node.addEventListener('pointerenter', handleWarmup, { once: true });
                node.addEventListener('focusin', handleWarmup, { once: true });
            };

            registerPointerWarmup(`.related-feature-card[data-id="${spot.id}"]`);
            registerPointerWarmup(`.related-feature-action[data-id="${spot.id}"]`);
            registerPointerWarmup(`.related-neighbor-card[data-id="${spot.id}"]`);
        });
    }
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    detailPage.readyPromise?.catch((error) => {
        console.error('[Yanqi detail] Detail page init failed:', error);
    });
    if (isStageDebugModeEnabled) {
